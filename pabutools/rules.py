...
def _phragmen_internal(e : Election, endow : dict[Voter, float], W : set[Candidate]) -> set[Candidate]:
    ...
    while True:
        ...
        for c in remaining:
            if costW + c.cost <= e.budget:
                time = float(c.cost - sum(endow[i] for i in e.profile[c])) / max(len(e.profile[c]), 1)
                if time < lowest_time:
                    next_candidate = c
                    lowest_time = time
...
def _mes_internal(e : Election, real_budget : int = 0) -> (dict[Voter, float], set[Candidate]):
    ...
    rho = {c : c.cost / max(sum(e.profile[c].values()),1) for c in e.profile}
    while True:
        next_candidate = None
        ...
        for c in remaining_sorted:
            ...
            if sum(endow[i] for i in e.profile[c]) >= c.cost:
                ...
                price = c.cost
                util = max(sum(e.profile[c].values()),1)
